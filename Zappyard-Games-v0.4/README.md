...
## v0.3 — tail engineering

The core rules remain unchanged.

Math changes:
- all cluster pays are 3% lower than v0.2
- Free Spins use **2.5× the normal powerup spawn odds**
- base-game powerup odds are unchanged

Purpose: move a small portion of RTP away from routine base wins and into rarer Free Spin chain events while preserving an overall RTP near 96%.

Validation: see `results/v0.3-tail-validation-500k.txt`.


## v0.4 — final pre-implementation math tune

The ordinary reward loop is intentionally preserved. Payouts for 5–15 symbol clusters are unchanged from v0.3.

Only the rare 16+ cluster bands were increased (6× v0.3) to move incremental RTP into the tail instead of making routine hits richer. In the 2M-spin validation, RTP measured **96.43588%** while hit frequency remained **33.20205%**.

See `results/v0.4-final-micro-tune-2m.txt`.

A 50,000× max-win constant is now included in the production specification. Final declared RTP still requires a much larger certification-style validation because the intentionally heavy tail increases Monte Carlo variance.

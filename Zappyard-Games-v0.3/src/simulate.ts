...
function bucket(win: number): keyof typeof buckets {
  ...
  if (win < 20) return "[5,20)x";
  if (win < 100) return "[20,100)x";
  if (win < 500) return "[100,500)x";
  if (win < 1000) return "[500,1000)x";
  if (win < 5000) return "[1000,5000)x";
  return "5000x+";
}

function featureLengthBucket(n: number): keyof typeof featureLengthBuckets {
  if (n <= 5) return "<=5";
  if (n <= 10) return "6-10";
...
const pct = (v: number) => `${(v * 100).toFixed(5)}%`;
